...
function RecentAlertItem({ alert }: { alert: Alert }) {
  ...
  return (
    <div className={cn('border-l-4 rounded-r-lg p-3 mb-2', severityBorder(alert.severity))}>
      ...
      <p className={cn('text-xs mt-1 ml-5',
...
export default function Dashboard() {
  ...
  // No-blink guard: only re-render when counts actually change
  ...
  const handleWS = useCallback((msg: WSMessage) => {
    if (msg.type === 'stats_update') {
      ...
      // Only trigger re-render when a number actually changed
      ...
      // Always accept fresh alert list from WS
    ...
    <div className="flex flex-col h-full overflow-y-auto">
      ...
      <div className="px-8 pt-8 pb-4 flex items-start justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">EmpowerHer Analytics — live session data</p>
        </div>
      ...
      <div className="px-8 pb-8 flex gap-5 flex-1 min-h-0">
        ...
        <div className="flex flex-col gap-5 flex-1 min-w-0">
          ...
          {/* Safety metrics — use session totals so bars actually fill */}
